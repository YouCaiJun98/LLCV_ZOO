...
class BSD(Dataset):
    def __init__(self, gray:bool, pth, length, patch_size, sigma, rnd_aug):
        ...
        assert self.patch_size % 2 == 0, "Odd patch size is not recommended" + \
            "for it prones to mismatch in feature dimensions while concating."
        ...
        for p in glob.glob(pth + '*[jpg, png]'):
            img = Image.open(p)
            img = np.array(rgb2gray(img))[..., np.newaxis] if self.gray else \
                np.array(img)
            # H_padding = (1, 0) if img.shape[0] % 2 != 0 else (0, 0)
            # W_padding = (1, 0) if img.shape[1] % 2 != 0 else (0, 0)
            # C_padding = (0, 0)
            # img = np.pad(img, (H_padding, W_padding, C_padding), mode='symmetric')
