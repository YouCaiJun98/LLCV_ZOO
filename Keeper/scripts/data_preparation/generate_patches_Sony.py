...
parser.add_argument('--src_dir', default='./datasets/Sony/Sony', type=str, help='Directory for full resolution images')
parser.add_argument('--tar_dir', default='./Sony_patches/',type=str, help='Directory for image patches')
...
noisy_patchDir = os.path.join(tar, 'short')
clean_patchDir = os.path.join(tar, 'long')
...
#get sorted folders
files = natsorted(glob(os.path.join(src, '*', '*.ARW')))
