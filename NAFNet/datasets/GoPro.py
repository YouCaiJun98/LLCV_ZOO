...
class GoPro_sRGB_Train_DataSet(Dataset):
    def __init__(self, path, patch_size, length=None):
        ...
        self.len = length if length else len(self.noisy_imgs)
    ...
    def __getitem__(self, index):
        ...
        # random augmentation
        noisy_patch = Data_Augmentation(self.transform(noisy_patch), rng)
        clean_patch = Data_Augmentation(self.transform(clean_patch), rng)
