...
def main():
    ...
    args.slave = False if args.local_rank == 0 else True
    if args.save_flag:
        ...
        save_list = ['models/nafnet.py'] + [__file__] + [args.configuration]
    ...
    scheduler_cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, epochs, eta_min=lr_schedule[-1])
    ...
    train_data = datasets.SID_Sony(args.root, img_list_files[0], patch_size=patch_size,  data_aug=True,  stage_in='sRGB', stage_out='sRGB')
    val_data   = datasets.SID_Sony(args.root, img_list_files[1], patch_size=512, data_aug=False, stage_in='sRGB', stage_out='sRGB')
    test_data  = datasets.SID_Sony(args.root, img_list_files[2], patch_size=512, data_aug=False, stage_in='sRGB', stage_out='sRGB')
    ...
    # SIDD only
    train_data = datasets.SIDD_sRGB_Train_DataLoader(os.path.join(args.root, 'train'), 96000, patch_size, True)
