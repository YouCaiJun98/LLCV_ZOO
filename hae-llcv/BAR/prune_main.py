...
def main():
    ...
    model = models.__dict__[args.arch](inchannel=3, outchannel=3, ratio=16).cuda()
    logging.info(model)
    if args.gpu_flag:
        model.cuda()
        criterion.cuda()
    ...
    train_data = datasets.SIDD_sRGB_Train_DataLoader(os.path.join(args.root, 'train_small'), 48000, 256, True)
